...
class CameraModule(mp_module.MPModule):
	...
	##TODO get rid of separate height and width and depend on camera params        
        ...
        self.add_command('camera', self.cmd_camera,
                         'camera control',
                         ['<start|stop|status|view|noview|boundary|airstart>',
                          'set (CAMERASETTING)'])
    ...
    def check_commands(self, bsend):
        ...
        if isinstance(obj, GetImageSettingBundle):
            ...
            for key in self.image_settings.list():
                bundle[key.name] = self.image_settings.get_setting(key.name)
        ...
        if isinstance(obj, GetCameraSettingBundle):
            ...
            for key in self.camera_settings.list():
                bundle[key.name] = self.camera_settings.get(key.name)
